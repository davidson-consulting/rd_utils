...
namespace rd_utils::utils {
    ...
    std::string* parse_string(tokenizer& tok, char end) {
		...
		while (true) {
			...
			} else if (!escape && next.length () == 1 && next[0] == '\\') {
				escape = true;
			} else if (!escape && next.length() == 1 && next[0] == end) {
				break;
    ...
    config::dict parse(const std::string& content) {
		...
		try {
			...
			while (true) {
				...
				if (next == "[") {
					auto name = tok.next();
					tokenizer nameTok (name, {":"},  {":", " ", "\t", "\n", "\r" }, {});
					auto currentDict = &dic;
					auto currentName = nameTok.next ();

					assert_msg(tok.next() == "]", "expected ]", tok);
					auto content = parse_dict(tok, true);

					while (true) {
						auto scdName = nameTok.next ();
						if (scdName != "") {
							if (currentDict-> has <config::dict> (currentName)) {
								auto nDic = new config::dict (currentDict-> getRef<config::dict> (currentName));
								currentDict-> insert (currentName, nDic,  typeid (config::dict).name());
								currentDict = nDic;
								currentName = scdName;
							} else {
								auto nDic = new config::dict ();
								currentDict-> insert (currentName, nDic,  typeid (config::dict).name());
								currentDict = nDic;
								currentName = scdName;
							}
							...
							currentDict-> insert (currentName, content, typeid (config::dict).name());
							break;
						}
					}
				}
			...
			std::stringstream ss;
			tok.rewind ();
			ss << "unexpected '" << tok.next () << "'";
			throw config::config_error(tok.getLineNumber(), ss.str () + " at line : " + std::to_string(tok.getLineNumber()));
		}
    }
...
std::ostream& operator << (std::ostream& stream, const rd_utils::utils::config::array& array) {
    array.print(stream);
    return stream;
}

std::ostream& operator << (std::ostream& stream, const rd_utils::utils::config::dict& array) {
    array.print(stream);
    return stream;
}
